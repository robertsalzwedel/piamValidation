---
title: "GCAM 8 Scenario MIP Validation Report"
...
regs.short <- c(
  "Central America and Caribbean" = "CA and Car",
  "European Free Trade Association" = "Europe FTA",
  "South America_Northern" = "SA North",
  "South America_Southern" = "SA South"
)
```
...
# Data Preparation
config <- paste0("scenarioMIP_", "GCAM71", "_", config_date)
...
hist <- readRDS(paste0("data/historical_all_", historical_date, ".rds")) %>%
  filter(grepl("GCAM71", region) | region == "World") 
...
and reformulated to be machine-readable bounds, with the exact values listed in a [**validationConfig**](https://pik-piam.github.io/piamValidation/articles/configs/validationConfig_ScenarioMIP_GCAM.html) file. For further information on the validation tool, consult the [ documentation](https://pik-piam.r-universe.dev/articles/piamValidation/validateScenarios.html) or the [accompanying paper](https://gmd.copernicus.org/articles/18/9897/2025/).
...
#### Line Plots
```{r, fig.width=9, fig.height=5}
model_lineplots(var, hist_models = c("IEA GEVO Historical", "IEA GEVO Historical_smoothed"))
```
